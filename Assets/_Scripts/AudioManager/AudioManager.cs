...
public static class AudioManager
{
    ...
    public static PoolingAudioSource Play(AudioAsset audioAsset, float pitch, float volume)
    {
        if (!PoolManager.Get<PoolingAudioSource>(_audioSourcePrefab, out var audioSource))
        {
            Debug.LogError("Fail to create audio source. Please check your resource floder");
        ...
        return audioSource.Play(audioAsset.AudioClip, pitch, volume, _audioGroups[audioAsset.Mixer]);
    }

    public static PoolingAudioSource Play(AudioAsset audioAsset)
    {
        if (!PoolManager.Get<PoolingAudioSource>(_audioSourcePrefab, out var audioSource))
        {
            Debug.LogError("Fail to create audio source. Please check your resource floder");
